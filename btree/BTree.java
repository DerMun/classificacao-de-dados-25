...
class BTreeNode {
	...
	*/
    ...
    void removeFromInternal(int idx) {
        ...
        if (children[idx].numKeys >= t) {
            System.out.println("[LOG] usando PREDECESSOR para substituir a chave " + k);
            ...
            System.out.println("[LOG] usando SUCESSOR para substituir a chave " + k);
            ...
            //se ambos tem t-1, fará merge. O log de merge está dentro do método merge()
            merge(idx);
            children[idx].remove(k);
        }
    }
    ...
    void borrowFromPrev(int idx) {
        System.out.println("[LOG] borrowFromPrev(): O filho " + idx + " pegou emprestado do irmão à ESQUERDA.");
    ...
    void borrowFromNext(int idx) {
        System.out.println("[LOG] borrowFromNext(): O filho " + idx + " pegou emprestado do irmão à DIREITA.");
        BTreeNode child = children[idx];
        BTreeNode sibling = children[idx + 1];
     ...
     */
    void merge(int idx) {
        System.out.println("[LOG] merge(): Realizando fusão (MERGE) no índice " + idx + " envolvendo chave " + keys[idx]);
        BTreeNode child = children[idx];
        BTreeNode sibling = children[idx + 1];
